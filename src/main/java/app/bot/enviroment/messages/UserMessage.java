...
public class UserMessage {
    ...
    public SendMessage pretendedToPay(StringBuilder builder,Long chatId) {
        builder.setLength(0);
        builder.append("Отлично. Мы создали для вас учетную запись. Тариф: 30 дней.\n\n")
                .append("Для согласования суммы напишите @biker_niga");
        return getSendMessage(chatId, builder.toString(), keyboard.startPay());
    }
    ...
    public SendMessage payUpdate(StringBuilder builder,Long chatUserId, int pay) {
        ...
        return getSendMessage(chatUserId,builder.toString(), keyboard.startPay());
    }
}
